...
class VlansManager(BaseManager):
    ...
    def get(self, interactive=False, retries=3, target=None):
        attempt = 0
        while attempt < retries:
            ...
            attempt += 1
        print("Failed to retrieve VLANs after several attempts.")
        return None

    def create(self, target, vlan_id=None, vlan_name=None, commit_directly=False):
        print("Creating VLAN.......\n")
        retry_limit = 3
        attempts = 0
        while attempts < retry_limit:
            try:
                existing_vlans, *_ = self.get(target=target)
            ...
            try:
                ...
                for vlan in existing_vlans['vlan']:
                    if vlan['vlan-id'] == vlan_update_id:
                        ...
                        payload.append(self.delete(choice=vlan_update_id, check_used=False, target=target))
    ...
    def delete(self, target, choice=None, check_used=True):
        print("Deleting VLAN...")
        existing_vlans, vlan_members = self.get(target=target)
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python vlans_manager.py <target>")
        sys.exit(1)
    target = sys.argv[1]
    main(target, VlansManager)
